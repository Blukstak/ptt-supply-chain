 ...
 * 3) Close-up: el mando final sale de la rueda → lupa roja en el EXTREMO del mando → de ahí se "desprende" el corte
 *    con los engranajes (R5 · obs. 00:34) → horas de operación.
 * 4) El componente sale de la faena en cama baja hacia el taller PTT.
 * R5 · fluidez: el mando final exterior, el corte y la cama baja viven en capas GPU (`artLayer`) y se mueven con
 * transform CSS del <svg>; el fondo (rajo, taller) no se vuelve a pintar en cada frame.
 */
export function faenaScene(): Scene {
  ...
  // Geometría del bloque técnico (R5 · obs. 00:34)
  const WX = 1320, WY = 560 // dónde queda la rueda trasera tras el zoom
  const EXT = { x: 1330, y: 560, s: 0.7 } // mando final exterior mientras giran los engranajes
  const END = { x: EXT.x - 203 * EXT.s, y: EXT.y } // centro del EXTREMO del mando (tapa `-fl1` en x=-203 local)
  const LENS_R = 82
  const CORTE = { x: 600, y: 560 } // corte con engranajes (finalDrive r=250 → carcasa r≈320)

  // Capa de fondo técnico: viñeta, anillo de pernos, título
  const fdBgSvg = fullSvg(`
    ...
    <g id="fd-ring" opacity="0" transform="translate(${WX} ${WY})"><circle r="150" fill="none" stroke="#e0262b" stroke-width="5" stroke-dasharray="14 10"/>${Array.from({ length: 12 }, (_, i) => `<circle cx="${Math.cos((i / 12) * Math.PI * 2) * 150}" cy="${Math.sin((i / 12) * Math.PI * 2) * 150}" r="9" fill="#e0262b"/>`).join('')}</g>
    <g id="fd-link" opacity="0"><text x="700" y="150" text-anchor="middle" font-family="Barlow Condensed, sans-serif" font-weight="700" font-size="44" fill="#e9eef4" letter-spacing="2">MANDO FINAL · CAT 797F</text><text x="700" y="190" text-anchor="middle" font-family="JetBrains Mono, monospace" font-size="16" fill="#9aa7b6" letter-spacing="3">COMPONENTE DE LA RUEDA TRASERA · EN EL EXTREMO, LOS ENGRANAJES</text></g>
  `)
  // Mando final exterior (capa GPU propia; ~800 dientes con gradientes: no se re-rasteriza al moverse)
  const fdExtSvg = artLayer(`<g transform="translate(700 560)">${finalDriveSide('fde', 'PTT')}</g>`, 700, 560)
  fdExtSvg.style.opacity = '0'
  // Corte con engranajes (capa GPU propia; nace en la lupa y se desprende hacia la izquierda)
  const fdGearSvg = artLayer(`<g transform="translate(${CORTE.x} ${CORTE.y})">${finalDrive('fd', 250)}</g>`, CORTE.x, CORTE.y)
  fdGearSvg.style.opacity = '0'
  // Lupa roja sobre el extremo + cono de conexión + rótulos + horómetro
  const fdSvg = fullSvg(`
    <g id="fd-lens" opacity="0" transform="translate(${END.x} ${END.y})">
      <circle r="${LENS_R}" fill="#e0262b" fill-opacity=".14" stroke="#e0262b" stroke-width="6"/>
      <circle id="fd-lens-pulse" r="${LENS_R}" fill="none" stroke="#e0262b" stroke-width="3" opacity="0"/>
    </g>
    <g id="fd-cone" opacity="0" stroke="#e0262b" stroke-width="3" stroke-dasharray="12 8" fill="none">
      <path id="fd-cone-a" d="M${END.x} ${END.y - LENS_R} L${CORTE.x} ${CORTE.y - 322}"/>
      <path id="fd-cone-b" d="M${END.x} ${END.y + LENS_R} L${CORTE.x} ${CORTE.y + 322}"/>
    </g>
    <g id="fd-callouts" opacity="0" font-family="JetBrains Mono, monospace" font-size="15" fill="#9aa7b6" letter-spacing="2">
      <path d="M${CORTE.x - 190} ${CORTE.y - 170} L${CORTE.x - 330} ${CORTE.y - 250} L${CORTE.x - 360} ${CORTE.y - 250}" stroke="#e0262b" stroke-width="2" fill="none"/><text x="${CORTE.x - 370}" y="${CORTE.y - 245}" text-anchor="end" fill="#e9eef4">CORONA (RING GEAR)</text>
      <path d="M${CORTE.x - 60} ${CORTE.y} L${CORTE.x - 330} ${CORTE.y} L${CORTE.x - 360} ${CORTE.y}" stroke="#e0262b" stroke-width="2" fill="none"/><text x="${CORTE.x - 370}" y="${CORTE.y + 5}" text-anchor="end" fill="#e9eef4">SOLAR (SUN GEAR)</text>
      <path d="M${CORTE.x - 170} ${CORTE.y + 180} L${CORTE.x - 330} ${CORTE.y + 250} L${CORTE.x - 360} ${CORTE.y + 250}" stroke="#e0262b" stroke-width="2" fill="none"/><text x="${CORTE.x - 370}" y="${CORTE.y + 255}" text-anchor="end" fill="#e9eef4">PORTA PLANETARIOS</text>
    </g>
    <g id="fd-gauge" transform="translate(1690 300)" opacity="0">${hoursGauge('g1', 140)}<text text-anchor="middle" y="${140 * 0.52}" font-family="JetBrains Mono, monospace" font-size="14" fill="#5f6d7d" letter-spacing="2">MÁX. 18.000 H</text></g>
  `)
  const semiSvg = artLayer(`<g transform="translate(600 720) scale(.95)">${lowboyTruck('semi')}<g transform="translate(330 140) scale(.42)">${finalDriveSide('cr', 'PTT')}</g></g>`, 600, 720)
  ...
  const tagLife = tag('Próximo a cumplir vida útil programada · cambio a las 18.000 h', 1180, 840)
  ...
  // R5 · obs. 00:34
  const tagEnd = tag('Los engranajes van en el extremo del mando final', 1010, 690, 'info')

  root.append(bg)
  photoBg(root, 'mine-day.jpg', bg)
  root.append(dust.canvas, shedSvg, truckSvg, fdBgSvg, fdExtSvg, fdGearSvg, fdSvg, semiSvg, lt1, tagParts, tagLife, tagGD, tagOut, tagEnd, q1)
  ...
  return {
    ...
    build(tl, at) {
      ...
      // el componente nace en la rueda (pequeño) y se desacopla: viaja al centro creciendo mientras el camión queda atrás
      tl.set(fdExtSvg, artAt(700, 560, WX, WY, 0.55), zoomAt)
      tl.to(fdExtSvg, { opacity: 1, duration: 0.4 }, zoomAt + 1.4)
      tl.to(fdExtSvg, { ...artAt(700, 560, WX - 40, WY, 0.62), duration: 0.4, ease: 'sine.inOut', repeat: 1, yoyo: true }, zoomAt + 1.4)
      pop(tl, tagOut, zoomAt + 1.7, 1.8)
      tl.to(fdExtSvg, { ...artAt(700, 560, 700, 560, 1.45), duration: 1.2, ease: 'power3.inOut' }, zoomAt + 2.2)
      ...
      // R5 · obs. 00:34 — el mando exterior se hace a un lado (visible, atenuado); una lupa roja se ancla en su EXTREMO
      //    y desde ahí se "desprende" el corte con los engranajes, unido por un cono de líneas.
      const gearsAt = zoomAt + 3.3
      tl.to(fdExtSvg, { ...artAt(700, 560, EXT.x, EXT.y, EXT.s), opacity: 0.6, duration: 0.9, ease: 'power3.inOut' }, gearsAt)
      tl.fromTo(q('#fd-lens'), { opacity: 0, attr: { transform: `translate(${END.x} ${END.y}) scale(.3)` } }, { opacity: 1, attr: { transform: `translate(${END.x} ${END.y}) scale(1)` }, duration: 0.5, ease: 'back.out(2)' }, gearsAt + 0.8)
      tl.fromTo(q('#fd-lens-pulse'), { opacity: 0.9, attr: { r: LENS_R } }, { opacity: 0, attr: { r: LENS_R + 60 }, duration: 1.1, repeat: 4, ease: 'power2.out' }, gearsAt + 1.1)
      pop(tl, tagEnd, gearsAt + 1.2)
      tl.to(q('#fd-cone'), { opacity: 1, duration: 0.2 }, gearsAt + 1.3)
      tl.fromTo([q('#fd-cone-a'), q('#fd-cone-b')], { strokeDasharray: 1000, strokeDashoffset: 1000 }, { strokeDashoffset: 0, duration: 0.7, ease: 'power2.inOut' }, gearsAt + 1.3)
      tl.set(fdGearSvg, artAt(CORTE.x, CORTE.y, END.x, END.y, 0.22), gearsAt)
      tl.to(fdGearSvg, { ...artAt(CORTE.x, CORTE.y, CORTE.x, CORTE.y, 1), opacity: 1, duration: 1.0, ease: 'power3.out' }, gearsAt + 1.4)
      tl.set([q('#fd-cone-a'), q('#fd-cone-b')], { strokeDasharray: '12 8' }, gearsAt + 2.1)
      ...
      tl.to(q('#fd-callouts'), { opacity: 1, duration: 0.6 }, gearsAt + 2.3)
      const gAt = gearsAt + 2.5
      ...
      tl.to(tagEnd, { opacity: 0, duration: 0.4 }, gAt + 1.6)
      pop(tl, tagLife, gAt + 1.8)

      // 4) El componente sale de la faena en cama baja hacia el taller PTT
      const rAt = gAt + 3.4
      tl.to([q('#fd-callouts'), q('#fd-gauge'), q('#fd-link'), q('#fd-lens'), q('#fd-cone'), fdGearSvg, fdExtSvg, tagLife, q('#fd-bgrect'), truckSvg], { opacity: 0, duration: 0.6 }, rAt - 0.5)
      ...
      tl.set(semiSvg, artAt(600, 720, -1000, 720, 1), at)
      tl.to(semiSvg, { ...artAt(600, 720, 2200, 720, 1), duration: 4.0, ease: 'power1.inOut' }, rAt)
      tl.to(['#semi-w0', '#semi-w1', '#semi-w2', '#semi-w3', '#semi-w4'].map(q), { rotation: 1400, transformOrigin: '50% 50%', duration: 4.0, ease: 'power1.inOut' }, rAt)
      pop(tl, tagGD, rAt + 0.6, 2.7)
      const end = revealQuote(tl, q1, rAt + 0.9, 1.9)
